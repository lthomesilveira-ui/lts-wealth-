...
## P0 — current release v160 — 05/09/2026
...
- [x] v157 WebKit-targeted recovery passed deterministic gates but was rejected by real iPhone evidence: financial KPIs unavailable, text overlap and clipped mobile navigation.
- [x] v158 removed the shadow financial Dashboard and recovered truthful native/auth fallback; corrected nested-core gates passed.
- [x] v159 improved prototype/mobile presentation and six-route fit, but real iPhone evidence rejected it because all five liquidity-first KPIs remained `—`.
- [x] v160 fixes the release-readiness defect: Dashboard success now requires material financial cockpit data, not merely completed patch/boot activity.
- [x] Candidate `wip35-v160-candidate.html` recursively scans the bounded same-origin app chain and uses the existing read-only RPC `lts_browser_dashboard_cockpit_v1` when cockpit data is not already available.
- [x] If material financial data is unavailable, the candidate must expose the native/login/core state rather than a prototype Dashboard full of dashes.
- [x] v160 owns the six candidate navigation controls exclusively: Dashboard, Fluxo Diário, Despesas, Patrimônio, Cartões, Atualizações.
- [x] No synthetic financial values, no financial writer, no `document.write`, permanent `setInterval` or `MutationObserver` introduced.
- [x] Source branch `v160-real-data-bridge`; final staged head `9f49c8637796d0e56b012896bedb55f5e29259c1`.
- [x] Branch v160 Chromium/WebKit real-data bridge gate `33991299085`: SUCCESS.
- [x] v160 integrated into main by normal fast-forward; no force.
- [x] Controlled homologation exposure commit `cbc4bc9d7e4798a9e97b80feb7510af535ac6d43`.
- [x] Exact-main v160 gate `33992238668`: SUCCESS.
- [x] Exact GitHub Pages deployment `33992238643`: SUCCESS.
- [x] `homologacao-current.json` points to v160 / `wip35-v160-candidate.html` / `promotion_status:not_promoted`.
- [x] Immutable checkpoint: `backups/WIP35_V160_REAL_DATA_BRIDGE_HOMOLOGATION_CHECKPOINT_2026-09-05.md`.
- [x] Public `index.html` remains protected and unchanged; public promotion is NOT authorized.
- [ ] Authenticated physical-iPhone financial/data E2E remains pending and must not be claimed; technical delivery to fixed homologation is complete.
...
- [x] v155 false-zero/navigation deterministic gate passed; v156 removed permanent runtime churn; v157 WebKit deterministic gate passed but was superseded by real-device failure evidence; v158 truthful native/auth recovery passed corrected gates.
- [x] v160 branch `33991299085`, exact-main `33992238668` and exact Pages `33992238643` passed; gate includes Chromium desktop + WebKit iPhone-equivalent five-KPI controlled bridge, six-route navigation, no clipping/overflow and truthful no-fixture fallback.
...
- [ ] Authenticated physical-iPhone financial/data E2E of current v160 when material real-device validation is required; deterministic technical delivery is complete and this must not be falsely claimed as already executed.
