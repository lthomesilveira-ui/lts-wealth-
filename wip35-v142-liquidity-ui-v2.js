(function(){
  ...
  function brNumber(raw){
    ...
    if(/(?:^|\s)mil$/.test(s)||/k$/.test(s)){mult=1000;s=s.replace(/(?:^|\s)mil$|k$/g,'').trim()}
  ...
  function parseAmount(text){
    ...
    for(const p of pats){const m=String(text||'').match(p);if(m){const n=brNumber((m[1]||'')+(m[2]?' '+m[2]:''));if(n)return n}}
    return null
  }
  ...
  function install(){
    ...
    w.__LTS_V142_PARSE_AMOUNT=parseAmount;
    w.__LTS_V142_PARSE_AMOUNT_SELFTEST={
      fiveMil:parseAmount('apliquei 5 mil no cofrinho')===5000,
      fiveThousandPt:parseAmount('resgatei R$ 5.000 do cofrinho')===5000,
      fiveThousandRaw:parseAmount('apliquei 5000 hoje')===5000,
      threeK:parseAmount('resgatei 3k do cofrinho')===3000,
      decimalPtBr:parseAmount('apliquei R$ 1.250,50')===1250.5
    };
    w.LTS_V142_LIQUIDITY_UI='reviewed-preview-confirm-v2';
  }
