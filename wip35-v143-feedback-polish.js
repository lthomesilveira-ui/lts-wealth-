(function(){
  ...
  function recurringPanel(w){
    const a=w.D?.updates?.recurring_future_audit||{},xs=a.planning_review_candidates||[],covered=(a.covered||[]).filter(x=>['Condomínio - O Parque','IPTU - O Parque','Benjamin - Educação','Adiantamento Quinzenal'].includes(String(x.description||''))).slice(0,4);
    ...
    return `<section class="v143fb" id="v143RecurringAudit"><div class="v143fb-head"><div><span>Assistente de futuro</span><h3>Futuro conferido contra o seu histórico</h3><p>O sistema procura séries que aparecem com frequência no passado e verifica se existe cobertura nos próximos 12 meses. Nada é criado automaticamente.</p></div><div class="v143fb-count">${xs.length}</div></div>${rows||'<div class="v143fb-note">Nenhuma recorrência relevante sem cobertura futura encontrada.</div>'}${covered.length?`<div class="v143fb-note"><b>Já cobertos no futuro</b><div class="v143fb-covered">${covered.map(x=>`<span>${esc(x.description)}</span>`).join('')}</div></div>`:''}<div class="v143fb-note">A ausência futura abre uma investigação: pode significar término, mudança de nome ou projeção faltante. O LTS não copia automaticamente a mediana histórica para o futuro.</div></section>`;
  }
  function injectUpdates(w,d){
    if(w.V!=='Atualizações'||!w.D)return;
    const old=d.getElementById('v143FeedbackUpdates');if(old)return;
  ...
  function focusDetail(w,d){
    let type='',el=null,key='';
    if(w.__V143_EXP_DRILL&&!w.__V143_EXP_DRILL.loading){type='expense';el=d.querySelector('.v143-drill');key=type+':'+String(w.__V143_EXP_DRILL.data?.key||w.__V143_EXP_DRILL.error||'done');}
    else if(w.__V143_CARD_DETAIL&&!w.__V143_CARD_DETAIL.loading){type='card';el=d.querySelector('[data-v143-card-close]')?.closest('.v143-panel');key=type+':'+String(w.__V143_CARD_DETAIL.card?.invoice_id||w.__V143_CARD_DETAIL.card?.card_name||'done');}
    else if(w.__V143_CIPO_OPEN){type='cipo';el=d.querySelector('[data-v143-cipo-close]')?.closest('.v143-panel');key='cipo:open';}
    else if(w.__V143_VEST_SCEN){type='scenario';el=d.querySelector('.v143-scenario');key='scenario:'+String(w.__V143_VEST_SCEN.id||'open');}
  ...
  function tick(){
    ...
    const w=z.w,d=z.d;w.LTS_V143_FEEDBACK_POLISH='classification-evidence-recurrence-focus-v1';css(d);bind(w,d);injectUpdates(w,d);focusDetail(w,d);wired=w;
  }
