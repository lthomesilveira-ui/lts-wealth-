...
Última auditoria material: 08/09/2026 12:27 BRT (America/Sao_Paulo)
...
| UPD-01 | Atualizações é classification-first conforme V147, sem vazios; `Lançamento por texto` mantém preview/revisão e documentos/gestão ficam em ações secundárias recolhidas. | V1.13 preserva a superfície v1.12, coloca classificação no foreground, cria hosts estáveis e corrige navegação de mesma rota sem remount. | Gates `34242926042` / `34243185722` validam hierarquia, detalhes fechado/aberto, âncoras, coexistência de módulos, fixture sem writer e desktop/mobile. | Uso real autenticado não fechado; V147/V150 são as origens comprovadas dos requisitos. | Save→refresh autenticado, mensagens de erro reais e iPhone físico. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, com `reviewed_input` e `document_intake` separados. | V1.13 recovery/candidate `34242926042` / `34242925889`; canonical gate/candidate `34243185722` / `34243185837`; main smoke/Pages `34243383377` / `34243381614`; pós-exposição `34243737853` / `34243795334` / `34244113857` / `34244111908`; artifacts `10062796446` e `10062906709`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `6a25aa99ab9548ef001870d9303d65700a0da8ec` aponta ao produto v1.13 `6d2b546b5e32d9ac09013cbd682e261e3f41ccac`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, assets `updates13`, login limpo, zero fixture financeira deslogada, nenhum iframe e nenhum erro da aplicação. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar v1.12 `70c49b1…` / exposição `adac528…` como baseline e manter seus gates de input revisado, camadas, verdade decisória e Flow parity;
...
3. manter Despesas v1.8 e Atualizações v1.12 regression-protected e avançar a próxima lacuna comprovada sem inventar um modelo de Relatórios não recuperado;
