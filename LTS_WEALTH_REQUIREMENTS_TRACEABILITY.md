...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, incluindo UX V1.19 e toda a linha V1.18→V1.9. | V1.19 recovery/candidate `34279813550` / `34279813574`; canonical gate/candidate `34281714299` / `34281714284`; pós-exposição `34282394183` / `34282421576` / `34282451603` / Pages `34282450565`; artifacts `10077205170` e `10077915479`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `59222846a5c7e67c923790da9ad4088df9781fa4` aponta ao produto V1.19 `37781c3b11389789716781accca6c1fdabea461c`; `index.html` intacto. | Gates pré/pós-exposição e Pages verdes; URL fixa verificou alvo exato, assets `ux19`, login limpo/associado/live, zero iframe e nenhuma mensagem técnica normal. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar V1.19 `37781c3b…` / exposição `59222846…` como baseline e manter toda a linha V1.18→V1.9 regression-protected;
