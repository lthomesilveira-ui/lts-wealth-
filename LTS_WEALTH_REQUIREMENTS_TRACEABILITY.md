...
Última auditoria material: 08/09/2026 13:35 BRT (America/Sao_Paulo)
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, com `reviewed_input`, `document_intake` e `document_review` separados. | V1.14 recovery/candidate `34250450654` / `34250450670`; canonical gate/candidate `34250649137` / `34250649010`; main smoke/Pages `34250977997` / `34250976391`; pós-exposição `34251224292` / `34251281745` / `34251333462` / `34251332488`; artifacts `10065805910` e `10065921441`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `ac5d4674920615c6c5e5947d4ab721f538679a0c` aponta ao produto v1.14 `0d50699af5aa5b950ff889a1fc6becfee209ce06`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, assets `doc-review14`, login limpo, zero fixture/JWT deslogado, nenhum iframe e nenhum erro da aplicação. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar v1.14 `0d50699…` / exposição `ac5d467…` como baseline e manter seus gates de revisão documental, intake, input revisado, camadas, verdade decisória e Flow parity;
...
3. manter Despesas v1.8 e Atualizações v1.14 regression-protected e avançar a próxima lacuna comprovada sem inventar um modelo de Relatórios não recuperado;
