...
Última auditoria material: 10/09/2026 — recuperação do Fluxo
...
- `REJEITADO_USUARIO`: implementação apresentada e recusada; testes históricos não a tornam baseline.
- `RECUPERACAO_EM_CURSO`: contrato recuperado e implementação/gate ainda não homologados pelo usuário.
...
Estado desta definição: `RECUPERACAO_EM_CURSO`. A v1.24 foi rejeitada pelo usuário e seus gates verdes são apenas evidência histórica. A entrega corrente recupera primeiro o Fluxo V150/V152; o produto completo ainda não substitui comprovadamente o Excel.

## Processo vinculante do próximo Dashboard

Nenhuma implementação de Dashboard é baseline atual. A referência e os contratos abaixo são insumos acumulados, mas o próximo Dashboard só pode ser codificado depois que uma imagem completa for apresentada e aprovada explicitamente pelo usuário. A proposta deve reconciliar:
...
| Canonical v1.19 Holistic UX Closure | Preserva v1.18 e fecha erro seguro, acessibilidade de estado/teclado, um único `Hoje` e legibilidade mobile. | `VERDE_AUTOMATICO`; produto `37781c3…` exposto por `5922284…`; E2E real/físico aberto. |
| Canonical v1.20 Flow Projection Recovery | Restaura a geometria V150/V151+ e as ações projetadas editar, postergar, duplicar, dividir/substituir e cancelar sem reabrir fatos. | `VERDE_AUTOMATICO`; protegido sob V1.23. |
| Canonical v1.21 Executive Reports | Reconstrói Relatórios como superfície executiva baseada somente nos leitores certificados. | `VERDE_AUTOMATICO`; protegido sob V1.24. |
| Canonical v1.22 Product Recovery | Refina hierarquia/leitura do Dashboard sem alterar valores, camadas ou gestão×primeiro negativo. | `VERDE_AUTOMATICO`; produto `181546c…` exposto por `3e3fa9a…`. |
| Canonical v1.23 Daily-Use Flow Closure | Fecha nove camadas progressivas mobile, verdade de fatura aberta/fechada e ciclo append-only completo de projeções. | `VERDE_AUTOMATICO`; produto `7f6bac39…` exposto por `d804eb0…`. |
| Canonical v1.24 Responsive Dashboard Readability | Tentou manter a referência 1312×1199 e criar composições para notebook, intermediário e mobile. | `REJEITADO_USUARIO` em 10/09/2026. Gates verdes ficam apenas como evidência técnica histórica. |
| V162 Daily Flow Recovery | Recupera diretamente o componente de Fluxo V150 preservado em `index.html`, usa V152 como referência, leitura atual V10 e não inclui Dashboard nesta homologação. | `VERDE_AUTOMATICO` no run `34495044417`; homologação humana e dados reais continuam separados. |
...
| ARC-01 | A aplicação final deve ser canônica e não depender de wrappers históricos. | V162 não executa a cadeia histórica; usa um frame isolado de homologação sobre o `index.html` inalterado e uma ponte de leitura limitada. | Gate rejeita qualquer wrapper V137–V152 no runtime e limita o escopo ao Fluxo. | Recuperação isolada autorizada pelo usuário; arquitetura final continua aberta. | Depois do aceite funcional, consolidar sem perder o comportamento homologado. |
...
| FLOW-01 | Fluxo passado/hoje/futuro, horizonte operacional de 90 dias, prioridade absoluta de confiabilidade. | Recuperação V162 restaura diretamente o contrato visual/funcional V150 preservado e usa `lts_browser_flow_v10`: 11 colunas operacionais, histórico contínuo, hoje, projeção e quatro contas. | Gate desktop/mobile `34495044417` passou; o run `34492222128` rejeitou a cadeia cumulativa por congelamento. | Usuário apontou V150/V152 como piso aceito e rejeitou a superfície posterior. | `VERDE_AUTOMATICO`; E2E autenticado físico e aceite humano permanecem abertos. |
...
| FLOW-03 | Transferência interna e banco↔ativo têm efeito econômico consolidado zero; pagamento de fatura não duplica despesa. | V162 mantém o evento de caixa separado da composição de compras e usa o mesmo detalhe `lts_browser_card_settlement_detail_v2` para Visa Aeternum e C6. | Gate V162 exige o mesmo resumo, categorias, pendência e detalhe nos dois cartões; regras antigas permanecem evidência auxiliar. | Regra reiterada pelo usuário; valores/documentos reais não são inferidos. | Testar faturas reais autenticadas após gate determinístico. |
| FLOW-04 | Projeção pode ser editada, postergada, duplicada, dividida/substituída ou cancelada sem apagar a origem. | A superfície V150 recuperada expõe editar, mudança de data, duplicar, dividir/substituir e excluir previsão; fatos históricos permanecem sem ações. | Gate V162 verifica presença das ações e ausência delas em fatos; writers não são chamados pela fixture. | Usuário reiterou editar, duplicar e excluir como mínimo obrigatório. | E2E autenticado das ações e readback permanece aberto. |
| FLOW-05 | Mobile deve manter o Fluxo legível e não transformar awards futuros em saldo corrente. | V162 recompõe cada dia sem overflow e preserva a escada V150: saldo operacional, D0/D1, vested e FGTS restrito. RSUs/Cash Awards futuros ficam fora da tabela principal. | Gate V162 `34495044417` em 390×844 mede overflow, os 10 campos além da data e prova zero antes do vesting e entrada somente na data de vesting da fixture. | Regra corrigida explicitamente pelo usuário em 10/09/2026. | `VERDE_AUTOMATICO`; confirmar em iPhone físico com dados reais. |
| DASH-01 | Dashboard deve ser visualmente e informacionalmente aprovado antes de implementação. | A implementação v1.24 foi rejeitada e está fora da candidata V162. A próxima etapa começa por uma imagem completa, não por código. | Gates v1.24 são retidos somente como histórico técnico; não constituem aceite visual. | `REJEITADO_USUARIO` para v1.24; nova imagem ainda não apresentada. | Produzir proposta após fechar o Fluxo e aguardar aprovação explícita. |
...
| DASH-06 | O Dashboard executivo aprovado deve caber inteiro no canvas desktop 1312×1199 sem eliminar camadas ou compromissos, sem impor microtipografia às demais telas. | V1.15 fecha a altura oficial; V1.24 limita essa densidade ao perfil de referência e dá tipografia/composição próprias a notebook, intermediário e mobile. | Gate mantém página oficial ≤1199 px e mede, por perfil, fontes, tracks, painéis/gráficos, ações e `scrollWidth === clientWidth`; recovery/canonical `34434661341` / `34435078818`. | A referência define o canvas oficial, não uma licença para texto de 6.5–8 px em notebooks. Mobile/intermediário continuam scrolláveis por decisão responsiva. | `VERDE_AUTOMATICO`; preservar como baseline e validar materialmente quando houver sessão real. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` com suíte funcional Chromium/WebKit e perfis responsivos dedicados de referência, notebook, intermediário e mobile, incluindo V1.24→V1.9. | V1.24 recovery/candidate `34434661341` / `34434661330`; canonical gate/candidate `34435078818` / `34435078779`; main/Pages `34435242921` / `34435242177`; pós-exposição smokes `34435460592` / `34435523608` / `34435568259` e Pages `34435567557`; artifacts `10135754220` e `10135897969`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| UX-01 | Mobile 390px, intermediário 1024px e desktop/notebook, sem overflow, travamento, observador recursivo ou UI escondida. | V1.19 preserva acessibilidade/touch/erro seguro; V1.24 adiciona `executive-readable-breakpoints-v1` com composição e tipografia mensuráveis por classe de viewport. | Chromium/WebKit sem churn/clipping; Dashboard 1312×1199, 1366×900, 1024×900 e 390×844; fontes operacionais e ações têm pisos por perfil. | iPhone real revelou falha de sessão fora do gate anterior. | `VERDE_AUTOMATICO`; revalidar apenas o E2E físico final. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | O manifesto ainda aponta à v1.24 rejeitada enquanto V162 passa pelo gate; `index.html` permanece intacto. | Exposição V162 fica bloqueada até gate desktop/mobile verde e verificação do alvo. | Usuário autorizou a recuperação do Fluxo, não a promoção do root público. | Expor V162 apenas em homologação; manter `not_promoted` e registrar aceite humano separadamente. |
...
| Canônica v1.24 | contrato `executive-readable-breakpoints-v1` | Preserva o canvas oficial 1312×1199 e fecha a legibilidade mensurável em notebook 1366×900, intermediário 1024×900 e mobile 390×844 sem alterar regra ou valor financeiro. |

## Pendências ordenadas sem depender do chat

### P0 executável agora

1. preservar V1.24 `39cb869f…` / exposição `084a3521…` como baseline e manter toda a linha V1.23→V1.9 regression-protected;
