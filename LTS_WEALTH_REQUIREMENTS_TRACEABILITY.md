...
Última auditoria material: 08/09/2026 14:31 BRT (America/Sao_Paulo)
...
| DASH-01 | Dashboard visualmente alinhado à referência oficial 1312×1199. | Pass 6 restaura hierarquia/conteúdo; V1.15 fecha o overflow vertical e materializa `approved-1312x1199-single-screen-v1`; não declara paridade pixel-perfect. | Gates `34256124696` e `34256598442`, artefatos desktop/mobile e smokes pós-exposição verdes. | Referência `VALIDADO_USUARIO`; URL fixa v1.15 verificada deslogada no alvo exato. | `ABERTO_ENGENHARIA`: detalhe pixel-level restante, gráficos históricos com evidência e E2E real. |
...
| UPD-01 | Atualizações é classification-first conforme V147, sem vazios; `Lançamento por texto` mantém preview/revisão e documentos/gestão ficam em ações secundárias recolhidas. | V1.14 preserva a superfície v1.13/v1.12, classificação no foreground, hosts estáveis e navegação de mesma rota sem remount. | Gates `34250450654` / `34250649137` preservam a hierarquia, coexistência dos módulos, fixture sem writer e desktop/mobile. | Uso real autenticado não fechado; V147/V150 são as origens comprovadas dos requisitos. | Save→refresh autenticado, mensagens de erro reais e iPhone físico. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, incluindo density, `reviewed_input`, `document_intake` e `document_review`. | V1.15 recovery/candidate `34256124696` / `34256124706`; canonical gate/candidate `34256598442` / `34256598444`; main smoke/Pages `34256889054` / `34256887644`; pós-exposição `34257179939` / `34257183053` / `34257139985` / `34257139795`; artifacts `10067974348` e `10068181842`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `b1998a9178ef3a8eec90dd47aa21e31dad8c868a` aponta ao produto v1.15 `a0a2675e6a9c9b3ffe4d9b0bc1fab222a48bc347`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, assets `dashboard-density15`, login limpo/campos vazios, zero fixture/JWT deslogado, nenhum iframe e nenhum erro da aplicação. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar v1.15 `a0a2675…` / exposição `b1998a9…` como baseline e manter seus gates de densidade 1312×1199, revisão documental, intake, input revisado, camadas, verdade decisória e Flow parity;
...
3. manter Dashboard v1.15, Despesas v1.8 e Atualizações v1.14 regression-protected e avançar a próxima lacuna comprovada sem inventar um modelo de Relatórios não recuperado;
