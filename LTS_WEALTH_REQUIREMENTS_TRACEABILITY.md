...
| V162 Daily Flow Recovery | Reabre a cadeia V152/V150 somente para recuperar o Fluxo aceito, com leitura atual V10 e sem Dashboard nesta homologação. | `RECUPERACAO_EM_CURSO`; gate determinístico e homologação humana ainda separados. |
...
| ARC-01 | A aplicação final deve ser canônica e não depender de wrappers históricos. | V162 usa a cadeia preservada somente como ponte temporária e isolada para recuperar/homologar o Fluxo V150; não altera o `index.html`. | Gate da recuperação registra a cadeia e limita o escopo ao Fluxo. | Exceção de recuperação autorizada pelo usuário; arquitetura final continua aberta. | Depois do aceite funcional, consolidar sem perder o comportamento homologado. |
...
| FLOW-01 | Fluxo passado/hoje/futuro, horizonte operacional de 90 dias, prioridade absoluta de confiabilidade. | Recuperação V162 restaura o contrato visual/funcional V150 sobre a cadeia V152 e usa `lts_browser_flow_v10`: 11 colunas operacionais, histórico contínuo, hoje, projeção e quatro contas. | Gate desktop/mobile V162 em execução; gates canônicos anteriores ficam apenas como histórico. | Usuário apontou V150/V152 como piso aceito e rejeitou a superfície posterior. | `RECUPERACAO_EM_CURSO`; E2E autenticado físico após gate determinístico. |
