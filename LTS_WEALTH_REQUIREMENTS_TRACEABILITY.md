...
| DASH-01 | Dashboard visualmente alinhado à referência oficial 1312×1199. | Pass 6 restaura hierarquia/conteúdo; V1.15 fecha o overflow vertical; V1.16 preserva exatamente o canvas e adiciona a decisão V151 sem alterar densidade. | Gates v1.16 `34262013499` e `34262279303`, artefatos desktop/mobile e smokes pós-exposição verdes. | Referência `VALIDADO_USUARIO`; URL fixa v1.16 verificada deslogada no alvo exato. | `ABERTO_ENGENHARIA`: detalhe pixel-level restante, gráficos históricos com evidência e E2E real. |
...
| UPD-01 | Atualizações é classification-first conforme V147, sem vazios; `Lançamento por texto` mantém preview/revisão e documentos/gestão ficam em ações secundárias recolhidas. | V1.16 preserva integralmente a superfície v1.14/v1.13/v1.12, classificação no foreground, hosts estáveis e navegação de mesma rota sem remount. | Gates v1.16 preservam a hierarquia, coexistência dos módulos, fixture sem writer e desktop/mobile. | Uso real autenticado não fechado; V147/V150 são as origens comprovadas dos requisitos. | Save→refresh autenticado, mensagens de erro reais e iPhone físico. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, incluindo planning v1.16, density, `reviewed_input`, `document_intake` e `document_review`. | V1.16 recovery/candidate `34262013499` / `34262013502`; canonical gate/candidate `34262279303` / `34262278639`; main smoke/Pages `34262576921` / `34262575286`; pós-exposição `34262885534` / `34262887519` / `34262889285` / `34262887576`; artifacts `10070303869` e `10070422301`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `d9a63c96e2674f7bf8d615e29643598bd8146537` aponta ao produto v1.16 `4028321fed599496d29798e3e2d1c47c48bc54d1`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, assets `planning16`, login limpo/campos vazios, zero fixture/JWT deslogado, nenhum iframe e nenhum erro da aplicação. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar v1.16 `4028321…` / exposição `d9a63c9…` como baseline e manter seus gates de decisão V151, densidade 1312×1199, revisão documental, intake, input revisado, camadas, verdade decisória e Flow parity;
2. continuar a convergência de detalhe do Dashboard preservando Pass 6, v1.9 V150 Flow parity, v1.10 observado×projetado / compromisso×tarefa e v1.11 base×RSU condicional×FGTS restrito;
3. manter Dashboard v1.16, Despesas v1.8 e Atualizações v1.14 regression-protected e avançar a próxima lacuna comprovada sem inventar um modelo de Relatórios não recuperado;
