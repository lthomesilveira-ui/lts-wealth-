...
Última auditoria material: 08/09/2026 11:16 BRT (America/Sao_Paulo)
...
| UPD-01 | Atualizações é action center compacto, sem vazios, com motivo claro e `Lançamento por texto` em preview/revisão explícita. | V1.12 restaura a superfície ausente; todos os campos são editáveis, preview não grava e writer real exige confirmação + idempotência + readback. | Gates `34235638676` / `34235911409` testam cinco formatos BRL, proposta semântica, aviso de lacunas, ausência de botão/writer na fixture e desktop/mobile. | Uso real autenticado não fechado; V150 é a origem comprovada do requisito. | Save→refresh autenticado, mensagens de erro reais e iPhone físico. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, agora com linha própria `reviewed_input`. | V1.12 recovery/candidate `34235638676` / `34235638646`; canonical gate/candidate `34235911409` / `34235911280`; main smoke/Pages `34236205811` / `34236204118`; pós-exposição `34236480960` / `34236541857` / `34236635343` / `34236635451`; artifacts `10059792070` e `10059891042`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `adac528dbde6d0dcd6268a52c780d3abf43dc9a9` aponta ao produto v1.12 `70c49b1b50b7faca09b71889e14d33183a7f9092`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, asset `input12`, login limpo, zero fixture financeira deslogada e nenhum iframe. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
