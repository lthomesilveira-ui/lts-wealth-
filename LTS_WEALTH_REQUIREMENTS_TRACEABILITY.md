...
Última auditoria material: 08/09/2026 15:33 BRT (America/Sao_Paulo)
...
- **Histórico recuperado:** 01/01/2026 zero/negativo, faixa amarela rejeitada em 24/08, diferença Bradesco R$0,01, evento Cofrinho 06/08, intervalo Bradesco R$0,17 em 12/08, hipótese Cofrinho R$186,10 e comparações 31/12/2026–31/12/2029 permanecem abertos até evidência; nenhum pode ser “resolvido” por inferência visual.
...
| HIST-01 | Anomalias 01/01/2026, faixa amarela 24/08 e diferenças Bradesco/Cofrinho devem ser explicadas por evidência, sem transformar hipótese em fato. | Ledger permanente consolidado; implementação corretiva ainda não concluída. | Checkpoint de recuperação impede fechamento silencioso. | Parte da sequência original não foi recuperada e os eventos R$0,17/R$186,10 exigem documento. | `ABERTO_ENGENHARIA` para 01/01/faixa/R$0,01; `ABERTO_DOCUMENTAL` para R$0,17 e R$186,10. |
| HIST-02 | Comparar saldos original×app em 31/12/2026 e 31/12/2029. | Requisito recuperado e preservado; nenhum resultado confiável localizado. | Deve receber gate/reconciliation receipt após cálculo sobre fontes reais. | Sem validação recuperada. | `ABERTO_ENGENHARIA`; não declarar paridade antes da comparação material. |
...
| DASH-01 | Dashboard visualmente alinhado à referência oficial 1312×1199. | Pass 6 restaura hierarquia/conteúdo; V1.15 fecha o overflow vertical; V1.16 adiciona a decisão V151; V1.17 preserva o canvas e remove linguagem técnica. | Gates v1.17 `34267339089` e `34267537086`, artefatos desktop/mobile e smokes pós-exposição verdes. | Referência `VALIDADO_USUARIO`; URL fixa v1.17 verificada deslogada no alvo exato. | `ABERTO_ENGENHARIA`: detalhe pixel-level restante, gráficos históricos com evidência e E2E real. |
...
| UPD-01 | Atualizações é classification-first conforme V147, sem vazios; `Lançamento por texto` mantém preview/revisão e documentos/gestão ficam em ações secundárias recolhidas. | V1.17 preserva integralmente a superfície v1.14/v1.13/v1.12, classificação no foreground, hosts estáveis, linguagem de produto e navegação de mesma rota sem remount. | Gates v1.17 preservam hierarquia, coexistência dos módulos, fixture sem writer, ausência de build técnico e desktop/mobile. | Uso real autenticado não fechado; V147/V150 são as origens comprovadas dos requisitos. | Save→refresh autenticado, mensagens de erro reais e iPhone físico. |
...
| QA-01 | QA automática antes do usuário; zero regressão; distinguir teste automático de validação real. | Gates permanentes geram `lts-canonical-definition-of-done-receipt-v1` em desktop Chromium e mobile WebKit, incluindo product-language v1.17, planning v1.16, density, `reviewed_input`, `document_intake` e `document_review`. | V1.17 recovery/candidate `34267339089` / `34267339099`; canonical gate/candidate `34267537086` / `34267537120`; main smoke/Pages `34267717067` / `34267715832`; pós-exposição `34267927974` / `34267929858` / `34267932211` / `34267929912`; artifacts `10072405055` e `10072479667`. | Usuário não deve ser QA botão a botão; iPhone físico autenticado segue não alegado. | `IMPLEMENTADO_PARCIAL`: recibo único existe e passa, mas linhas autenticadas/reais/físicas permanecem OPEN. |
...
| REL-01 | Homologação fixa preservada; root público separado e protegido. | Manifesto exposto em `06fdcbba8f1cbcae6af2ebf153a2291915195f96` aponta ao produto v1.17 `e4cea67563ac379d5bf47d02d20846699194a002`; `index.html` intacto. | Gates pré e pós-exposição/Pages verdes; URL fixa verificou alvo exato, assets `ux17`, login limpo/campos vazios, zero fixture/build técnico/JWT deslogado, nenhum iframe e nenhum erro da aplicação. | Promoção não autorizada. | Continuar `not_promoted` até autorização explícita. |
...
1. preservar v1.17 `e4cea675…` / exposição `06fdcbba…` como baseline e manter linguagem de produto, decisão V151, densidade 1312×1199, revisão documental, intake, input revisado, camadas, verdade decisória e Flow parity;
2. fechar as auditorias 01/01/2026, faixa 24/08, Bradesco/Cofrinho e 31/12/2026–31/12/2029 usando evidência real, sem inferência;
3. continuar a convergência de detalhe do Dashboard preservando Pass 6, v1.9 V150 Flow parity, v1.10 observado×projetado / compromisso×tarefa e v1.11 base×RSU condicional×FGTS restrito;
4. manter Dashboard v1.17, Despesas v1.8 e Atualizações v1.14 regression-protected e concluir a auditoria UX integral sem inventar um modelo de Relatórios não recuperado;
5. manter rota/sessão e o recibo único regression-protected, fechando novas linhas automáticas quando surgirem requisitos;
6. manter o novo baseline Supabase regression-protected e revisar leaked-password protection em uma janela controlada de Auth.
