(function(){
  ...
  function dashboardRuntime(){
    ...
    function dashboardView(){
      ...
      return '<main class="dx1" data-dashboard-contract="executive-cash-answer-evidence-v1">'+
        ...
        '<section class="dx1-decision '+decisionClass+'"><div class="dx1-answer"><span>A pergunta principal</span><h2>Tenho dinheiro suficiente?</h2><strong>'+txt(decision.answer)+'</strong><p>'+txt(decision.detail)+'</p><i>Cenário-base operacional</i></div><div class="dx1-answer-grid">'+
          metric('Primeiro saldo negativo',null,firstNegative).replace('Indisponível</b>',''+txt(firstNegative)+'</b>')+
          metric('Caixa coberto até',null,covered).replace('Indisponível</b>',''+txt(covered)+'</b>')+
          metric('Pior saldo projetado',decision.worst,worstNote)+
          metric('Ponto de gestão',null,pa.managementDate?'Reavaliar em '+date(pa.managementDate):'Não informado').replace('Indisponível</b>',txt(pa.managementDate?date(pa.managementDate):'Indisponível')+'</b>')+
        '</div></section>'+ 
    ...
    function applyInitial(){if(state.initialApplied||!D)return false;state.initialApplied=true;status.initial_applied_at=new Date().toISOString();V='Dashboard';renderNav();render();return true}
    ...
    const style=document.createElement('style');style.id='lts-dashboard-executive-style';style.textContent=`
      ...
      @media(max-width:420px){.dx1-answer-grid{grid-template-columns:1fr}.dx1-kpis{grid-template-columns:1fr}.dx1-kpi:first-child{grid-column:auto}.dx1-metric{padding:12px}.dx1-mobile-nav button span{font-size:6.5px}}
