(function(){
  ...
  function dashboardRuntime(){
    ...
    function applyInitial(){const flow=window.__LTS_V162_FLOW_RECOVERY_STATUS;if(state.initialApplied||!D||flow?.route!=='Fluxo Diário'||flow?.last_flow_done!==true)return false;state.initialApplied=true;status.initial_applied_at=new Date().toISOString();status.flow_ready_before_initial=true;V='Dashboard';renderNav();render();return true}
