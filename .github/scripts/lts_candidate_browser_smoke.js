...
(async()=>{
  ...
  const inner=f;
  const innerState=await inner.evaluate(()=>({
  ...
  })).catch(()=>({candidate:null,css138:false,css139:false}));
  const injectionsOk=!!innerState.css138&&!!innerState.css139&&String(innerState.candidate||'').includes('v139');
  const body=await page.locator('body').innerText().catch(()=> '');
  const result={pass:chainOk&&injectionsOk&&newErrors.length===0,chain,chainOk,injectionsOk,innerState,baselineErrors,candidateErrors,newErrors,topBodyChars:body.length};
