...
async function run(browser,viewport,label,{loginAfterLoad=false}={}){
  ...
  await route('Patrimônio').click();await frame.locator('.v165-wealth').waitFor({state:'visible'});await frame.locator('[data-v165-wealth-tab="rsu"]').click();await frame.locator('.v165-award').first().waitFor({state:'visible'});text=await frame.locator('.v165-wealth').innerText();sectionLower=text.toLocaleLowerCase('pt-BR');for(const phrase of ['rsus e awards','rsu já vested','cash rsus · líquido','70% após imposto de 30%','100% bruto; sem corte automático','02/11/2030','10/02/2031','15/03/2033','retention award','r$ 90.000,00','tratamento separado.'])if(!sectionLower.includes(phrase))throw Error(`${label}: wealth missing ${phrase}`);
