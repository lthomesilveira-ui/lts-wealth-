...
const semantic=value=>String(value||'').replace(/\s+/g,' ').trim().toLocaleLowerCase('pt-BR');
...
async function run(browser,viewport,label,{loginAfterLoad=false}={}){
  ...
  for(const phrase of ['Coberto até 19/12/2030','primeiro dia negativo em 20/12/2030','Resgatar FGTS até 20/11/2030','nova falta em 25/01/2031'])if(!dashboardText.includes(phrase))throw Error(`${label}: dashboard missing ${phrase}; kpis=${JSON.stringify(dashboardKpis)}`);
  const dashboardLower=semantic(dashboardText);
  if(!dashboardLower.includes('dados até 14/06/2030'))throw Error(`${label}: compact source date missing`);
  ...
  await route('Despesas').click();await frame.locator('.v165-expenses').waitFor({state:'visible'});await frame.locator('.v165-expenses .v165-kpi').first().waitFor({state:'visible'});let text=await frame.locator('.v165-expenses').innerText(),sectionLower=semantic(text);for(const phrase of ['visão geral','categorias','cartões','lançamentos','classificação revisada','100%','cobertura documental'])if(!sectionLower.includes(phrase))throw Error(`${label}: expenses missing ${phrase}`);
  await frame.locator('[data-v165-expense-tab="categories"]').click();text=await frame.locator('.v165-expenses').innerText();sectionLower=semantic(text);if(!sectionLower.includes('benjamin')||!sectionLower.includes('educação'))throw Error(`${label}: Benjamin hierarchy missing`);
  await frame.locator('[data-v165-expense-tab="cards"]').click();text=await frame.locator('.v165-expenses').innerText();sectionLower=semantic(text);for(const phrase of ['faturas abertas','uso mensal dos cartões','cartão principal','uma compra, uma despesa.'])if(!sectionLower.includes(phrase))throw Error(`${label}: cards-in-expenses missing ${phrase}`);
  if(label!=='post-login')await page.screenshot({path:`v165-${label}-expenses.png`,fullPage:true});
  await route('Patrimônio').click();await frame.locator('.v165-wealth').waitFor({state:'visible'});await frame.locator('.v165-tabs [data-v165-wealth-tab="rsu"]').click();await frame.locator('.v165-award').first().waitFor({state:'visible'});text=await frame.locator('.v165-wealth').innerText();sectionLower=semantic(text);for(const phrase of ['rsus e awards','rsu já vested','cash rsus · líquido','70% após imposto de 30%','100% bruto; sem corte automático','02/11/2030','10/02/2031','15/03/2033','retention award','r$ 90.000,00','tratamento separado.'])if(!sectionLower.includes(phrase))throw Error(`${label}: wealth missing ${phrase}`);
  if(await frame.locator('.v165-award').count()!==10)throw Error(`${label}: expected ten vesting rows`);
  if(label==='desktop'){const first=frame.locator('.v165-award').first(),input=first.locator('input[type="date"]');await input.fill('2030-10-15');await first.locator('[data-v165-award-save]').click();await frame.waitForFunction(()=>String(document.querySelector('.v165-award .v165-scenario-result')?.textContent||'').includes('Com antecipação'));if(!semantic(await first.innerText()).includes('com antecipação'))throw Error('desktop: anticipation result missing')}
  if(label!=='post-login')await page.screenshot({path:`v165-${label}-wealth.png`,fullPage:true});
  await route('Atualizações').click();await frame.locator('#v165SearchInput').fill('retention');await frame.waitForFunction(()=>window.__LTS_V165_STATE?.search?.loading===false&&window.__LTS_V165_STATE?.search?.total===3);text=await frame.locator('.v165-updates').innerText();sectionLower=semantic(text);for(const phrase of ['encontre sem saber a data','data e valor são opcionais','retention award','r$ 21.000,00','classificação concluída no escopo revisado'])if(!sectionLower.includes(phrase))throw Error(`${label}: updates search missing ${phrase}`);
