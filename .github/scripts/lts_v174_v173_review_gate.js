...
async function run(browser,viewport,label){
  ...
  await frame.waitForFunction(()=>window.__LTS_V169_STATE?.monthly?.data?.version==='monthly-balance-v4-v174-resilient-chunked',{timeout:30000});
  const ms=await frame.evaluate(()=>({months:window.__LTS_V169_STATE.monthly.data.months,calls:window.__LTS_V174_STATE.monthlyChunkCalls}));
