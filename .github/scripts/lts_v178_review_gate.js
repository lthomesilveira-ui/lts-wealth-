...
async function run(browser,viewport,label){
 ...
 try{
  ...
  await nav('Despesas').click();await frame.locator('[data-v168-exp-range="6m"]').click();await frame.locator('[data-v168-exp-tab="categories"]').click();await frame.waitForSelector('.v178-rank');
  ...
  await frame.locator('[data-v168-exp-range="all"]').click();await frame.locator('[data-v168-exp-tab="monthly"]').click();await frame.waitForFunction(()=>window.__LTS_V175_STATE.monthly.data?.months?.length===156&&!window.__LTS_V175_STATE.monthly.loading);
