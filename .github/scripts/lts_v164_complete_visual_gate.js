...
function expensePayload(from,to){
  const monthly=[['2026-01-01',41000],['2026-02-01',43800],['2026-03-01',45500],['2026-04-01',47100],['2026-05-01',46200],['2026-06-01',48800],['2026-07-01',49600],['2026-08-01',50896.09],['2026-09-01',46458.15]].map(([month,total])=>({month,total,account_total:total*.55,card_total:total*.45,rows:12,is_partial:month==='2026-09-01',top_categories:[{name:'Financiamentos',total:total*.4},{name:'Saúde',total:total*.2}],category_detail_pct:76,transaction_detail_pct:70}));
  const selected=monthly.reduce((n,x)=>n+x.total,0);
  const window=(count)=>{const rows=monthly.slice(-count),total=rows.reduce((n,x)=>n+x.total,0);return {total,monthly_average:total/rows.length,financing_total:total*.4,known_category_total:total*.76,category_known_total:total*.76,category_unallocated_total:total*.24,category_detail_pct:76,transaction_detail_pct:70,top_categories:[{name:'Financiamentos',total:total*.4},{name:'Saúde',total:total*.2},{name:'Educação',total:total*.12}]}};
  return {version:'expense-executive-v12-cached-effective-contract',period:{from,to,last_available:'2026-09-15',months_in_selection:monthly.length},summary:{selected_total:selected,account_total:selected*.55,card_total:selected*.45,monthly_average:selected/monthly.length,history_total:752104.87},monthly_detail:monthly,yearly_detail:[{year:2025,total:580000,monthly_average:48333,comparison_pct:5,financing_total:170000,top_categories:[],category_detail_pct:72,transaction_detail_pct:66},{year:2026,total:selected,monthly_average:selected/monthly.length,comparison_pct:-3,financing_total:selected*.4,top_categories:[],category_detail_pct:76,transaction_detail_pct:70,is_partial_year:true}],rolling_windows:{'3m':window(3),'6m':window(6),'12m':window(9)},category_trends_12m:[],data_quality:{selected_category_unallocated_total:selected*.24,selected_category_detail_pct:76,selected_transaction_detail_pct:70,selected_category_allocated_history_total:selected*.76},source_precision:{notice:''}};
}
