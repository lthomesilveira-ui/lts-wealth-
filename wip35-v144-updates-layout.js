(function(){
  ...
  const MARK='classification-action-center-v1';
  ...
  function evidenceText(x){
    return String(x?.enrichment_evidence||x?.amount_pattern_evidence||'').trim();
  }

  function enhanceCardRows(w,d){
    const items=w.D?.card_classification_review?.items||[];
    const byKey=new Map(items.map(x=>[String(x?.description_key||''),x]));
    d.querySelectorAll('#cardClassReview .cardclass-row').forEach(row=>{
      const btn=row.querySelector('.cardclass-save[data-key]');
      const key=String(btn?.dataset?.key||'');
      const x=byKey.get(key);if(!x)return;
      row.classList.add('v144-row-enhanced');
      let strip=row.querySelector('.v144-proofline');if(strip)strip.remove();
      let proof=row.querySelector('.v144-prooftext');if(proof)proof.remove();
      strip=d.createElement('div');strip.className='v144-proofline';
      const sc=n(x.suggestion_confidence),ec=n(x.enrichment_confidence);
      const parts=[];
      ...
      parts.push(`<span class="source">${esc(sourceLabel(x))}</span>`);
      strip.innerHTML=parts.join('');
      const main=row.firstElementChild||row;
      const signals=main.querySelector('.u138-signals');
      if(signals)signals.insertAdjacentElement('afterend',strip);else main.appendChild(strip);
      const ev=evidenceText(x);
      if(ev){proof=d.createElement('div');proof.className='v144-prooftext';proof.innerHTML=`<b>Pesquisa / evidência</b>${esc(ev)}`;strip.insertAdjacentElement('afterend',proof);}
    });
  }

  function enhanceExpenseRows(w,d){
    const items=w.D?.semantic_review?.items||[];
    const byKey=new Map(items.map(x=>[String(x?.description_key||''),x]));
    d.querySelectorAll('#expenseClassReview .cardclass-row').forEach(row=>{
      ...
      row.classList.add('v144-row-enhanced');
      if(row.querySelector('.v144-proofline'))return;
      const strip=d.createElement('div');strip.className='v144-proofline';
      const sc=n(x.suggestion_confidence),src=sourceLabel(x);
      strip.innerHTML=(sc>0?`<span class="${confidenceClass(sc)}">${Math.round(sc*100)}% sugestão</span>`:'<span>sem % conclusivo</span>')+`<span class="source">${esc(src)}</span>`;
  ...
  function actionCount(w){
    const xs=(w.D?.updates?.items||[]).slice();
    try{return typeof w.updateIsAction==='function'?xs.filter(w.updateIsAction).length:xs.length}catch(e){return xs.length}
  }
  ...
  function build(w,d){
    ...
    css(d);enhanceCardRows(w,d);enhanceExpenseRows(w,d);
    const existing=d.getElementById('v144UpdatesRoot');
    if(existing){w.LTS_V144_UPDATES_LAYOUT=MARK;return;}
    ...
    const card=d.getElementById('cardClassReview'),expense=d.getElementById('expenseClassReview');
    ...
    if(evidence)evidence.classList.add('v144-duplicate-hidden');
    if(feedback)feedback.classList.add('v144-feedback-reduced');

    const root=d.createElement('div');root.id='v144UpdatesRoot';
    ...
    if(card&&expense){const gap=d.createElement('div');gap.className='v144-class-gap';zone.appendChild(gap)}
    if(expense)zone.appendChild(expense);
    if(!card&&!expense){const empty=d.createElement('div');empty.className='v144-empty';empty.textContent='Nenhuma classificação pendente neste momento.';zone.appendChild(empty)}
    ...
    const body=other.querySelector('.v144-fold-body');
    Array.from(host.children).forEach(x=>{if(x!==root&&x!==card&&x!==expense&&x!==recurring)body.appendChild(x)});
    if(!body.children.length){const empty=d.createElement('div');empty.className='v144-empty';empty.textContent='Nenhum bloco secundário aberto.';body.appendChild(empty)}
    ...
    w.LTS_V144_UPDATES_LAYOUT=MARK;
  }
