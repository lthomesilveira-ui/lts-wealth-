...
- Fixed URL was browser-verified at `canonical-app.html?homologacao=8a9c675…#Dashboard`, loading `canonical-liquidity.js?v=20260908-dashboard-layers11`, truthful signed-out login, zero iframes and no unauthenticated fixture Dashboard. Authenticated real-data/write and physical-iPhone evidence remain open and are not claimed.
- V1.10 remains the protected predecessor for fact/projection and commitment/task separation; v1.11 does not change its `product-commitments-plus-card-due` source or the v1.9 Flow contract.
...
- Latest immutable evidence: `backups/CANONICAL_APP_V111_LAYERED_LIQUIDITY_CHECKPOINT_2026-09-08.md`.
...
1. Treat canonical v1.11 product `8a9c675…` and exposure `04c051f…` as the current protected baseline; do not regress its layered-liquidity contract, the v1.10 decision-truth contract, the v1.9 V150 Flow contract or public `index.html`.
...
6. Keep the v1.8 single-owner Despesas route, v1.9 V150 Flow parity, v1.10 fact/projection/commitment semantics and v1.11 liquidity layers regression-protected; continue remaining product quality and execute real write/search/document/backup-restore lifecycles when an authenticated browser identity is available.
