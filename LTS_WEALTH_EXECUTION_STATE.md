...
## Current execution state — 08/09/2026 09:10 BRT
- Repo: `lthomesilveira-ui/lts-wealth-`; active recovery branch remains `canonical-v157plus-product-recovery`.
- `main`, `canonical-app-v1` and `canonical-v157plus-product-recovery` are freshness-verified aligned at v1.9 documentation closure `3858739e73ebdedffbe06766c05fdbc9c59f91d6`; fixed manifest exposure `90aad3a754e69708fcc9e03a5bb32835281e2925` selects exact product `6aba220653a9053cf304ab11bf931db38ccc1b75`; public `index.html` remains untouched.
- Canonical v1.10 Dashboard Decision Truth is prepared from that exact base under contract `reference-fact-projection-commitments-v2`. It separates the observed position from later projections in both liquidity charts, uses a dashed projected series and explicit cut-off legend, and sources `Próximos Compromissos` only from the card due date plus `product.commitments.commitments`; review/classification/planning tasks remain exclusively in `Atualizações`.
- The v1.10 permanent local gate is PASS in Chromium desktop 1312×1199 and Chromium mobile 390×844 after physically exercising both charts, three documentary commitment rows, the commitment→Flow route, all six product routes, V150 Flow interactions and mobile overflow. CI WebKit and remote publication are pending for this candidate and must not yet be claimed.
...
- Latest immutable evidence: `backups/CANONICAL_APP_V19_V150_FLOW_PARITY_CHECKPOINT_2026-09-08.md`.
...
3. Publish canonical v1.10 Dashboard Decision Truth to the active recovery branch, require recovery/candidate Chromium+WebKit gates, then promote by normal no-force refs to canonical/main only if green; never copy financial values from the reference.
