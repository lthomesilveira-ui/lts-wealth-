...
## Current execution state — 08/09/2026 09:29 BRT
- Repo: `lthomesilveira-ui/lts-wealth-`; active recovery branch remains `canonical-v157plus-product-recovery`.
- Canonical v1.10 exact product is `de4362d90560424a332f77ce22e665729cd7d88f`; fixed-manifest exposure is `661be37ea45425a30dd3a35c7605d1921db94559`. This documentation/checkpoint closure is the next shared fast-forward for `main`, `canonical-app-v1` and `canonical-v157plus-product-recovery`; public `index.html` remains untouched.
- Dashboard Decision Truth contract `reference-fact-projection-commitments-v2` separates observed position from later projections in both liquidity charts, uses a dashed projected series and explicit cut-off legend, and sources `Próximos Compromissos` only from the card due date plus `product.commitments.commitments` rows with explicit `next_due`; review/classification/planning tasks remain exclusively in `Atualizações`.
- V1.10 pre-exposure recovery gate/smoke `34225155424` / `34225155422`, canonical gate/smoke `34225365210` / `34225365141`, main smoke/Pages `34225542803` / `34225541791`: SUCCESS. Post-exposure active/canonical/main smokes `34225917363` / `34225967201` / `34226016981` and Pages `34226016118`: SUCCESS.
- Recovery artifact `10055417140`, digest `sha256:46b285161dd778294e9d5161bbe6faa71d96e7bb31188b6b25bcc5adb608e33b`; canonical artifact `10055499807`, digest `sha256:a255c1a04e8a5b7f33ef5a199ad3b24c34fdd1bf8ed7f05245026f520373a841`.
- Fixed URL was browser-verified at `canonical-app.html?homologacao=de4362d…#Dashboard`, loading `canonical-liquidity.js?v=20260908-dashboard-truth10`, truthful signed-out login, zero iframes and no unauthenticated fixture Dashboard. Authenticated real-data/write and physical-iPhone evidence remain open and are not claimed.
...
- Latest immutable evidence: `backups/CANONICAL_APP_V110_DASHBOARD_DECISION_TRUTH_CHECKPOINT_2026-09-08.md`.
...
1. Treat canonical v1.9 exposure `90aad3a…` and product `6aba220…` as the protected baseline; do not regress the V150 Flow contract or change public `index.html`.
...
3. Treat canonical v1.10 product `de4362d…` and exposure `661be37…` as the protected Dashboard baseline; continue evidence-led detail work without copying financial values from the reference.
...
6. Keep the v1.8 single-owner Despesas route, v1.9 V150 Flow-parity contract and v1.10 fact/projection/commitment semantics regression-protected; continue remaining product quality and execute real write/search/document/backup-restore lifecycles when an authenticated browser identity is available.
