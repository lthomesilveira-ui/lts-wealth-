...
## Current execution state — 08/09/2026 08:22 BRT
- Repo: `lthomesilveira-ui/lts-wealth-`; active recovery branch remains `canonical-v157plus-product-recovery`.
- `main`, `canonical-app-v1` and `canonical-v157plus-product-recovery` are freshness-verified aligned at canonical v1.9 product commit `6aba220653a9053cf304ab11bf931db38ccc1b75`; this release package advances the fixed homologation manifest to that exact product while leaving public `index.html` untouched.
...
- Live Pages verification of product SHA `6aba220…` loaded `canonical-liquidity.js?v=20260908-flow-parity9`, rendered the truthful signed-out login and exposed no fixture financial content. Authenticated data/write behavior and a physical iPhone remain separate, explicitly open evidence.
...
1. Publish the canonical v1.9 fixed-manifest exposure across the three aligned branches, require post-exposure smokes/Pages to pass and verify the fixed URL resolves to product SHA `6aba220…`; do not change public `index.html`.
