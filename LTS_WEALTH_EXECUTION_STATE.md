...
- V1.19 preserves the V1.18 Flow history/horizon corrections, V1.17 product language, V1.16 planning distinction, V1.15 density, V1.14 V149 review, V1.13 hierarchy/intake, V1.12 reviewed input, V1.11 liquidity layers, V1.10 decision truth and the rich V150/V151+ Flow interactions.
- Recovery gate/smoke `34279813550` / `34279813574`, canonical gate/smoke `34281714299` / `34281714284`, pre-manifest main smoke/Pages `34281930573` / `34281929787`, post-manifest active/canonical/main smokes `34282394183` / `34282421576` / `34282451603` and Pages `34282450565`: SUCCESS.
- Recovery artifact `10077205170`, digest `sha256:799e68e527c299577940324894de8384ec27985f42c435a666c2e22125869170`; canonical artifact `10077915479`, digest `sha256:9a57901ea993b6d7ad1c0a80f2a8b95f00ce8bc812a40a3f4c6a2e475af5c224`.
- Visual evidence covers Chromium desktop 1312×1199 and WebKit mobile 390×844 across Dashboard, Flow and Updates. Fixed URL resolves to `canonical-app.html?homologacao=37781c3b11389789716781accca6c1fdabea461c`, loads `?v=20260908-ux19`, renders a truthful blank signed-out login with associated fields/live feedback and uses zero iframes.
- Latest immutable evidence: `backups/CANONICAL_APP_V119_UX_CLOSURE_CHECKPOINT_2026-09-08.md`; recovered two-month ledger: `backups/TWO_MONTH_FEEDBACK_RECOVERY_CHECKPOINT_2026-09-08.md`.
...
## Prior canonical v1.18 state — retained as evidence
- Exact remote product `692535276fe7948c386349254151538c0c9eddf8`; fixed-manifest exposure `65cb675b613b70b7a26f97f86938a0b3ac72b584`.
- Contracts `historical-opening-from-close-and-net-v1`, `today-marker-without-row-band-v1` and `future-through-2029-plus-d30-v1` close the recovered 01/01 opening, rejected yellow-row treatment and 2029/D+30 horizon gaps without changing a financial fact.
- Supabase migration `canonical_flow_horizon_and_slice_guard_2026_09_08` is applied; immutable evidence: `backups/CANONICAL_APP_V118_FLOW_HISTORY_HORIZON_CHECKPOINT_2026-09-08.md`.
...
- Contract `v151-first-negative-management-separation-v1` separates the exact first projected negative date from the earlier management-action date and never interpolates a missing negative value.
- Product recovery/canonical gates `34262013499` / `34262279303` and all associated candidate, Pages and post-exposure smokes passed.
- Immutable evidence: `backups/CANONICAL_APP_V116_PLANNING_DECISION_CHECKPOINT_2026-09-08.md`.
...
- The review is explicitly `Somente leitura`, contains no control and cannot approve, reconcile, classify, replace association or write a financial fact. Browser reader `lts_browser_document_review_queue_v1` is guarded; internal `lts_document_review_queue_v1` is owner-scoped, bounded and service-role-only.
- Migration `canonical_document_review_read_model_2026_09_08` is applied; zero-UUID and unauthenticated probes passed their expected read/no-write and `42501 unauthorized` boundaries.
...
- Immutable evidence: `backups/CANONICAL_APP_V114_DOCUMENT_INTERPRETATION_REVIEW_CHECKPOINT_2026-09-08.md`.
...
- Guided intake contract `classification-first-guided-document-intake-v1` requires explicit type-specific association before upload: bank→institution/account+competence; card→card+competence; financing→commitment+exact as-of; other→optional note/manual review. Filename never supplies facts and upload never auto-posts.
- Real confirmed files target private bucket `lts-documents`, then register through `lts_browser_register_document_v2`; failed registration attempts cleanup. The 50 MB/type guard runs before transfer. Fixture mode disables file/upload and proves `writer_called:false` / `write_accepted:false`.
- Same-route target navigation now opens the enclosing details element and scrolls without rerendering `Atualizações`; this fixes the remount bug that could erase reviewed-input/liquidity/management modules when opening Documentos from the current route.
- V1.13 preserves v1.12 reviewed input, v1.11 layered liquidity, v1.10 decision truth, v1.9 V150/V151+ Flow parity, v1.8 single-owner Despesas, route/session continuity and the Supabase security baseline.
...
- Fixed URL browser verification resolved to `canonical-app.html?homologacao=6d2b546…#Dashboard`, loaded both `?v=20260908-updates13` assets, rendered truthful signed-out login, used zero iframes and exposed no fixture KPI values or application errors.
- Latest immutable evidence: `backups/CANONICAL_APP_V113_CLASSIFICATION_FIRST_DOCUMENT_INTAKE_CHECKPOINT_2026-09-08.md`.
- Authenticated real upload→register→lifecycle/readback, authenticated financial writes/search, physical-iPhone E2E and public-root promotion remain open/not claimed. No approved standalone Reports visual was recovered, so none may be invented.
- Public `index.html` remains protected at blob `a130eafe5f7ee5b7f60a95b5ff988669d0c401d9`; public-root promotion is not authorized.

## Prior canonical v1.12 state — retained as evidence
- Repo: `lthomesilveira-ui/lts-wealth-`; active recovery branch remains `canonical-v157plus-product-recovery`.
...
5. Keep authenticated physical-iPhone E2E explicitly unclaimed until actually performed.
6. Treat the bounded Supabase RLS/SECURITY DEFINER package as the protected security baseline; leaked-password protection remains a separate controlled Auth-setting change.
