...
(function(){
 ...
 function runtime(){
  ...
  const state=window.__LTS_V178_STATE={cash:{status:'idle'},wealth:{status:'idle'},forecast:{status:'idle'},awards:{status:'idle'},dashboardReport:{status:'idle'},detail:null,errors:[],date:null};
  ...
  function logError(name,error){state.errors.push({reader:name,message:String(error?.message||error),at:new Date().toISOString()});state.errors=state.errors.slice(-30)}
  async function request(name,args={},timeout=20000,force=false){
   ...
   if(!token)throw Error('Sessão indisponível. Entre novamente.');
   ...
   const job=(async()=>{
    ...
    try{
     ...
     const data=await res.json();if(!res.ok)throw Error(data?.message||'Falha na leitura.');
     cache.set(key,{time:Date.now(),data});return data;
    }catch(error){logError(name,error);throw error}finally{clearTimeout(timer)}
   })();
  ...
  async function loadState(key,name,args={},force=false){
   ...
   const token=(s.token||0)+1;s.token=token;s.status='loading';s.error=null;
   try{const value=await request(name,args,20000,force);if(s.token!==token)return;s.data=value;s.date=day();s.status='ready'}
   catch(e){if(s.token===token){s.status='error';s.error=String(e.message||e)}}
   finally{if(s.token===token)queueMicrotask(()=>{if(D&&!N.classList.contains('hidden'))render()})}
  }
  ...
  function startDashboard(force=false){
   const t=day();
   if(state.date!==t){state.date=t;for(const k of ['cash','forecast','dashboardReport']){state[k].token=(state[k].token||0)+1;state[k].status='idle';state[k].data=null}cache.clear()}
   const r=yearRange();
   loadState('cash','lts_browser_cash_today_v178',{},force);
  ...
  function dashboard178(){
   ...
   const note=state.cash.status==='loading'?'Carregando a posição de hoje':complete?'Posição calculada no Fluxo · '+dateLabel(c.as_of):'Posição indisponível';
   ...
   const warnings=[];
   if(!complete)warnings.push(state.cash.status==='loading'?'Atualizando a posição de caixa…':'Não foi possível obter a posição completa de caixa.');
  ...
  window.__LTS_V178_REVIEW={installed:true,version:'v178',baseline:'v177',cashComplete,openDetail,closeDetail,refresh:()=>startDashboard(true)};
