...
function mark(extra){window.__LTS_CANONICAL_RECOVERY_STATUS={ready:true,build:BUILD,core_contract:CORE_CONTRACT.length,flow_contract:FLOW_CONTRACT.length,product_contract:PRODUCT_CONTRACT.length,flow_loaded:!!document.getElementById('canonicalFlowV157'),product_loaded:!!document.getElementById('canonicalProductV157'),polish_loaded:!!document.getElementById('canonicalV157Polish'),...(extra||{})}}
function ensureModules(){
  if(modulesPromise)return modulesPromise;
  modulesPromise=Promise.all([
    load('canonical-product-v157.js?v=20260907-v157plus-product-2','canonicalProductV157'),
    load('canonical-flow-v157.js?v=20260907-v157plus-product-2','canonicalFlowV157')
  ]).then(()=>load('canonical-v157-polish.js?v=20260907-v157plus-product-2','canonicalV157Polish')).then(()=>{mark({flow_loaded:true,product_loaded:true,polish_loaded:true});return true}).catch(e=>{modulesPromise=null;mark({error:String(e?.message||e)});throw e});
...
(async()=>{try{await load('canonical-liquidity-core.js?v=20260907-v157plus-product-2','canonicalLiquidityCore');mark({flow_loaded:false,product_loaded:false,polish_loaded:false});waitReady(0).catch(e=>mark({error:String(e?.message||e)}))}catch(e){console.error('LTS module loader',e);window.__LTS_CANONICAL_RECOVERY_STATUS={ready:false,build:BUILD,error:String(e?.message||e)}}})();
})();